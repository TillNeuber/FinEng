...
#for(term in c(12)) {
  ...
  while(!isTRUE(valid) || var < 0) {
    ...
    if(integral$value - integral$abs.error - epsilon <= 1 && 1 <= integral$value + integral$abs.error + epsilon) {
      ...
      mean<-integrate(function(x) {return(returnImplDensityUnscaled(x))}, 0.5, 5000)$value
      var<-(integrate(function(x) {return(returnImplDensityUnscaled(x)^2)}, 0.5, 5000)$value)/(term/12) - mean^2
    } else {
      valid<-FALSE
    }
