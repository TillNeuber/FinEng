...
plot(termvec, meanTermStructureSeries[1, ])

for (i in 2:nrow(meanTermStructureSeries)) {
  ...
  #abline(myline.fit)
  lines(termvec, lw$fitted,col="red")
}



volframe<-data.frame(termvec, volvec)
ggplot(volframe, aes(termvec,volvec)) + geom_point() + geom_smooth()
